...
#' loci=dat$loci,
#' alleles=dat$alleles,
#' S0=c("pop1","pop2"),	#Two populations in the \code{POPID} column chosen as parental reference set \code{S0}#
#' S1="pop3",			#One population in the \code{POPID} column chosen as parental reference set \code{S1}#
#' precols=dat$precols,
...
data.prep= function(data,loci,alleles,S0,S1,precols,
    ...
    max.S.MAF=0.5,min.diff=0,min.allele.copies.S0=0,min.allele.copies.S1=0,
    ...
    if(return.locus.table==TRUE){
        locus.data=unique(DT.join[,.(locus,S0_allele,S1_allele,S0.prop_1,S1.prop_1,S0.N_1,S1.N_1,S0.prop_0,
            S1.prop_0,N_allele_copies.S0,N_allele_copies.S1,Source.afdiff,S0.MAF,S1.MAF,min.MAF)]);
